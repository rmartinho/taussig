...
libtaussig = 'bin/libtaussig.a'
ninja.build(libtaussig, 'lib',
        inputs = obj_files)
...
ninja.build(test_runner, 'link',
        inputs = test_obj_files + [libtaussig])
ninja.build('test', 'phony',
        inputs = test_runner)
ninja.build('lib', 'phony',
        inputs = libtaussig)

tar = 'dist/taussig.tar.bz2'
ninja.build(tar, 'dist',
        inputs = hdr_files + ([libtaussig] if src_files else []))
